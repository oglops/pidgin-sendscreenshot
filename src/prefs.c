...
get_plugin_pref_frame (PurplePlugin * plugin)
{
  ...
  pidgin_prefs_labeled_entry (vbox,
			      PREF_UI_STORE_FOLDER, PREF_STORE_FOLDER, NULL);
#endif
	  ...
	  if (host_data->host_names->len > 0)
	    {
	      ...
	      gtk_label_set_markup (GTK_LABEL (hostlist_label), ui_upload_to);
	      g_free (ui_upload_to);
	      ui_upload_to = NULL;
	      ...
	      g_signal_connect_swapped (hostnames_link,
					...
					G_CALLBACK (website_link_clicked_cb),
					plugin);
		...
		gtk_combo_box_new_with_model (GTK_TREE_MODEL (list_store));
	      ...
	      g_signal_connect (G_OBJECT (hosts_combobox), "changed",
				G_CALLBACK (hosts_combobox_changed_cb), NULL);

	      hbox_html = gtk_hbox_new (FALSE, PIDGIN_HIG_CAT_SPACE);

	      gtk_box_pack_start (GTK_BOX (vbox), hbox_html, TRUE, FALSE, 0);
	      gtk_box_pack_start (GTK_BOX (hbox_html),
